...
import 'package:dental_app/features/archived_visits/presentation/widgets/ratingDialog.dart';
import 'package:dental_app/features/financial_and_billing/presentation/pages/financial_page.dart';
import 'package:dental_app/features/home/presentation/utils/home_rating_session_gate.dart';
import 'package:dental_app/features/home/presentation/widgets/active_treatment_plan_card.dart';
import 'package:dental_app/features/home/presentation/widgets/animated_notification_icon.dart';
import 'package:dental_app/features/home/presentation/widgets/assistant_hero_card.dart';
import 'package:dental_app/features/home/presentation/widgets/daily_tip_card.dart';
...
class HomePage extends StatelessWidget {
  ...
  /// يزيد عند فتح التطبيق / الرجوع من الخلفية — فرصة واحدةحدة لعرض التقييم.
  final int ratingCheckTick;

  /// true فقط لما تاب الرئيسية هو الظاهر (ما نعرض الديالوج فوق شاشات ثانية).
...
class _HomePageViewState extends State<_HomePageView> {
  ...
  void didUpdateWidget(covariant _HomePageView oldWidget) {
    ...
    if (oldWidget.ratingCheckTick != widget.ratingCheckTick) {
      // رجوع من الخلفية: حدّث العداد فقط — لا تعيد فتح ديالوج التقييم.
      _loadUnreadCount();
    }
  }
  ...
  Future<void> _handlePatientHome(PatientHomeSuccess state) async {
    await _tryShowRatingPrompt(state);
  }

  Future<void> _tryShowRatingPrompt(PatientHomeSuccess state) async {
    if (_dialogOpen || !mounted) return;
    // مرة واحدةحدة لكل تشغيل للتطبيق — حتى لو انعاد بناء الصفحة.
    if (HomeRatingSessionGate.handledThisLaunch) return;
    if (!widget.isHomeTabActive) return;
    ...
    // اقفل فوراً قبل أي await حتى ما ينفتح مرتين من طلبات متزامنة.
    HomeRatingSessionGate.handledThisLaunch = true;

    if (!await PatientStatusGuard.ensureSelectedPatientEditable(context)) {
      return;
    }
    if (!mounted || !widget.isHomeTabActive) return;
    ...
    // X بدون تقييم — ما عاد يظهر لهالتشغيل (لحد ما يسكر التطبيق ويفتحه من جديد).
                              ...
                              child: GridView.count(
                                ...
                                children: [
                                  ...
                                  QuickActionCard(
                                    ...
                                    subtitle: 'Pick a date & time'.tr(),
                                    onTap: () async {
                                      if (!await PatientStatusGuard.ensureSelectedPatientEditable(
                                        context,
                                      )) {
                                        return;
                                      }
